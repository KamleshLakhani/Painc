class PainIntensityEnjoymentReport {
  bool status;
  List<Data> data;
  List<PainIntensity> painIntensity;

  PainIntensityEnjoymentReport({this.status, this.data, this.painIntensity});

  PainIntensityEnjoymentReport.fromJson(Map<String, dynamic> json) {
    status = json['status'];
    if (json['data'] != null) {
      data = new List<Data>();
      json['data'].forEach((v) {
...
class Data {
  ...
  Data({this.date, this.generalActivity, this.enjoymentLife});
