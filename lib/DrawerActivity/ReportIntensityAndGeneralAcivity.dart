...
import 'dart:convert';
class PainIntensityAndGeneralActivity extends StatefulWidget {
  final String fromdate;
  final String todate;
  PainIntensityAndGeneralActivity({this.todate, this.fromdate});
  @override
  _PainIntensityAndGeneralActivityState createState() => _PainIntensityAndGeneralActivityState();
}

class _PainIntensityAndGeneralActivityState extends State<PainIntensityAndGeneralActivity> {
  List<_SalesData> data = [];
  List<_SalesData> data2 = [];
  Future<PainIntensityEnjoymentReport> intensitygeneralreport;
  @override
  void initState() {
    intensitygeneralreport = getdata();
    super.initState();
  }
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: CustAppbar(
        txt: 'Intensity & General Activity',
        infobtn: false,
      ),
      body: Container(
                ...
                Padding(padding: EdgeInsets.only(top:20,left: screenwidth*.12),
                    child: Column(
                      children: [
                        ...
                        Utils.axistitle(Color(0xFFFF9A3D),'General Activity '),
                      ],
                    )),
              ],
            ),
  ...
  Future<PainIntensityEnjoymentReport> getdata() async{
    ...
    print(response.statusCode);
      ...
      // PainIntensityEnjoymentReport pigeneralreport = PainIntensityEnjoymentReport.fromJson(json.decode(response.body));
      if(overallreport.status){
        overallreport.data.forEach((element) {
          ...
          if(element.painIntensity != 0 || element.generalActivity != 0){
            setState(() {
      ...
      // pigeneralreport.painIntensity.forEach((element) {
      //   var datee = element.date;
      //   String dateWithT = datee.substring(0, 8) + datee.substring(8);
      //   DateTime dateTime = DateTime.parse(dateWithT);
      //   String formattedDate = DateFormat('MMM-dd').format(dateTime);
